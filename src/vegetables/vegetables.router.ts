...
import { create, getAll, getById, remove, update } from "./vegetables.service";
...
router.get("/:id", async (req: Request, res: Response) => {
  const vegetable = await getById(req.params.id)
...
router.post("/", validationPost, async (req: RequestPost, res: Response) => {
  ...
  try {
    ...
    const result = await create(req.body.name, req.body.color, req.body.price)
...
router.put("/:id", validationPut, async (req: RequestPut, res: Response) => {
  ...
  const result = await update(req.params.id, req.body.name, req.body.color, req.body.price);
